...
In a stochastic process, elements of a set are each classified as being in one of several fixed states that can switch over time.
The switching to a particular state is described by a probability.
If the probability that an object in one state changes to a different state in a fixed interval of time depends only on the two states (and not on the time, earlier states, or other factors), then the stochastic process is called a Markov process.
If, in addition, the number of possible states is finite, then the Markov process is called a Markov chain.

Let $$ A $$ be the transition matrix and $$ P $$ be the initial probability vector of the Markov chain, then $$ \displaystyle \lim_{m \to \infty} A^m P $$ gives the eventual proportions in each state.
...
Fortunately, there is a large and important class of transition matrices for which this limit exists and is easily computed—this is the class of regular transition matrices.
...
Let $$ A \in \mathsf{M}_{n \times n}(C) $$.
For $$ 1 \le i, \ j \le n $$, define $$ \rho_i(A) $$ to be the sum of the absolute values of the entries of row $$ i $$ of $$ A $$, and define $$ \nu_j(A) $$ to be equal to the sum of the absolute values of the entries of column $$ j $$ of $$ A $$.
Thus

$$
\begin{align*}
    \rho_i(A) = \sum_{j = 1}^n \vert A_{ij} \vert && \text{ for } i = 1, 2, \dots, n
\end{align*}
$$
...
$$
\begin{align*}
    \nu_j(A) = \sum_{i = 1}^n \vert A_{ij} \vert && \text{ for } j = 1, 2, \dots, n
\end{align*}
$$
...
$$
\begin{align*}
    \rho(A) = \max\{\rho_i(A): 1 \le i \le n \} && \text{and} && \nu(A) = \max\{ \nu_j(A): 1 \le j \le n \}
\end{align*}
$$

To obtain a geometric view of the following theorem, we introduce some terminology.
For an $$ n \times n $$ matrix $$ A $$, we define the $$ i $$th Gershgorin disk $$ C_i $$ to be the disk in the complex plane with center $$ A_{ii} $$ and radius $$ r_i = \rho_i(A) - \vert A_{ii} \vert $$; that is,

$$
\begin{align*}
    C_i = \{ z \in C : \vert z - A_{ii} \vert \le r_i \}
\end{align*}
$$
...
Let $$ A \in \mathsf{M}_{n \times n}(C) $$.
Then every eigenvalue of $$ A $$ is contained in a Gershgorin disk.
...
$$
\begin{align*}
    \vert \lambda \vert \le \min \{ \rho(A), \nu(A) \}
\end{align*}
$$

**Proof**  
Since $$ \vert \lambda \vert \le \rho(A) $$ by Corollary 1, it suffices to show that $$ \vert \lambda \vert \le \nu(A) $$.
Since $$ \lambda $$ is an eigenvalue of $$ A^t $$, $$ \vert \lambda \vert \le \rho(A^t) $$ by Corollary 1.
But the rows of $$ A^t $$ are the columns of $$ A $$; consequently $$ \rho(A^t) = \nu(A) $$.
Therefore $$ \vert \lambda \vert \le \nu(A) $$. $$ \blacksquare $$

### Corollary 3

If $$ \lambda $$ is an eigenvalue of a transition matrix, then $$ \vert \lambda \vert \le 1 $$.
...
**Proof**  
Let $$ A $$ be an $$ n \times n $$ transition matrix, and let $$ u \in \mathsf{R}^n $$ be the column vector in which each coordinate is $$ 1 $$.
Then $$ A^t u = u $$ by Theorem 5.14, and hence $$ u $$ is an eigenvector of $$ A^t $$ corresponding to the eigenvalue $$ 1 $$.
But since $$ A $$ and $$ A^t $$ have the same eigenvalues, it follows that $$ 1 $$ is also eigenvalue of $$ A $$. $$ \blacksquare $$
...
For in this situation, $$ A^m P = P $$ for every positive integer $$ m $$; hence the probability of being in each state never changes.

### Theorem 5.17

Let $$ A \in \mathsf{M}_{n \times n}(C) $$ be a matrix in which each entry is a positive real number, and let $$ \lambda $$ be a complex eigenvalue of $$ A $$ such that $$ \vert \lambda \vert = \rho(A) $$.
Then $$ \lambda = \rho(A) $$ and $$ \{ u \} $$ is a basis for $$ \mathsf{E}_{\lambda} $$, where $$ u \in \mathsf{C}^n $$ is the column vector in which each coordinate equals $$ 1 $$.

**Proof**  
Let $$ v $$ be an eigenvector of $$ A $$ corresponding to $$ \lambda $$, with coordinates $$ v_1, v_2, \dots, v_n $$.
Suppose that $$ v_k $$ is the coordinate of $$ v $$ having the largest absolute value, and let $$ b = \vert v_k \vert $$.
Then
...
and hence $$ \{ u \} $$ is a basis for $$ \mathsf{E}_{\lambda} $$.  
Finally, we observe that all of the entries of $$ Au $$ are positive because the same is true for the entries of both $$ A $$ and $$ u $$.
But $$ Au = \lambda u $$, and hence $$ \lambda > 0 $$.
Therefore, $$ \lambda = \vert \lambda \vert = \rho(A) $$. $$ \blacksquare $$

### Corollary 1.

Let $$ A \in \mathsf{M}_{n \times n}(C) $$ be a matrix in which each entry is positive, and let $$ \lambda $$ be an eigenvalue of $$ A $$ such that $$ \vert \lambda \vert = \nu(A) $$.
Then $$ \lambda = \nu(A) $$, and the dimension of $$ \mathsf{E}_{\lambda} $$ equals $$ 1 $$.

### Corollary 2.

Let $$ A \in \mathsf{M}_{n \times n}(C) $$ be a transition matrix in which each entry is positive, and let $$ \lambda $$ be any eigenvalue of $$ A $$ other than $$ 1 $$.
Then $$ \vert \lambda \vert < 1 $$.
...
Then  
(a) $$ \vert \lambda \vert \le 1 $$  
(b) If $$ \vert \lambda \vert = 1 $$, then $$ \lambda = 1 $$, and $$ \dim(\mathsf{E}_{\lambda}) = 1 $$

**Proof**  
Statement (a) was proved as Corollary 3 to Theorem 5.15.  
(b) Since $$ A $$ is regular, there exists a positive integer $$ s $$ such that $$ A^s $$ has only positive entries.
Because $$ A $$ is a transition matrix and the entries of $$ A^s $$ are positive, the entries of $$ A^{s + 1} = A^s (A) $$ are positive.
Suppose that $$ \vert \lambda \vert = 1 $$.
Then $$ {\lambda}^s $$ and $$ {\lambda}^{s + 1} $$ are eigenvalues of $$ A^s $$ and $$ A^{s + 1} $$, respectively, having absolute value $$ 1 $$.
So by Corollary 2 to Theorem 5.17, $$ {\lambda}^s = {\lambda}^{s + 1} = 1 $$.
Thus $$ \lambda = 1 $$.
Let $$ \mathsf{E}_{\lambda} $$ and $$ {\mathsf{E}'}_{\lambda} $$ denote the eigenspaces of $$ A $$ and $$ A^s $$, respectively, corresponding to $$ \lambda = 1 $$.
Then $$ \mathsf{E}_{\lambda} \subseteq {\mathsf{E}'}_{\lambda} $$ and, by Corollary 2 to Theorem 5.17, $$ \dim({\mathsf{E}'}_{\lambda}) = 1 $$.
Hence $$ \mathsf{E}_{\lambda} = {\mathsf{E}'}_{\lambda} $$, and $$ \dim(\mathsf{E}_{\lambda}) = 1 $$. $$ \blacksquare $$
...
Let $$ A $$ be a regular transition matrix.
Then  
(a) The multiplicity of $$ 1 $$ as an eigenvalue of $$ A $$ is $$ 1 $$.  
(b) $$ \displaystyle \lim_{m \to \infty} A^m $$ exists.  
(c) $$ \displaystyle L = \lim_{m \to \infty} A^m $$ is a transition matrix.  
(d) $$ AL = LA = L $$.  
(e) The columns of $$ L $$ are identical.
...
(f) For any probability vector $$ w $$, $$ \displaystyle \lim_{m \to \infty} (A^m w) = v $$.

**Proof**  
(b) This follows from (a) and Theorems 5.18 and 5.12.  
(c) By Theorem 5.14, we must show that $$ u^t L = u^t $$.
Now $$ A^m $$ is a transition matrix by the corollary to Theorem 5.14, so

$$
\begin{align*}
    u^t L = u^t \lim_{m \to \infty} A^m = \lim_{m \to \infty} u^t A^m = \lim_{m \to \infty} u^t = u^t
\end{align*}
$$
...
$$
\begin{align*}
    AL = A \lim_{m \to \infty} A^m = \lim_{m \to \infty} A A^m = \lim_{m \to \infty} A^{m + 1} = L
\end{align*}
$$

Similarly, $$ LA = L $$.  
(e) Since $$ AL = L $$ by (d), each column of $$ L $$ is an eigenvector of $$ A $$ corresponding to the eigenvalue $$ 1 $$.
Moreover, by (c), each column of $$ L $$ is a probability vector.
Thus, by (a), each column of $$ L $$ is equal to the unique probability vector $$ v $$ corresponding to the eigenvalue $$ 1 $$ of $$ A $$.  
(f) Let $$ w $$ be any probability vector, and set $$ \displaystyle y = \lim_{m \to \infty} A^m w = Lw $$.
Then $$ y $$ is a probability vector by the corollary to Theorem 5.14, and also $$ Ay = ALw = Lw = y $$ by (d).
Hence $$ y $$ is also an eigenvector corresponding to the eigenvalue $$ 1 $$ of $$ A $$.
So $$ y = v $$ by (e). $$ \blacksquare $$
...
Let $$ \mathsf{T} $$ be a linear operator on a vector space $$ \mathsf{V} $$.
A subspace $$ \mathsf{W} $$ of $$ \mathsf{V} $$ is called a $$ \mathsf{T} $$-invariant subspace of $$ \mathsf{V} $$ if $$ \mathsf{T}(\mathsf{W}) \subseteq \mathsf{W} $$, that is, if $$ \mathsf{T}(v) \in \mathsf{W} $$ for all $$ v \in \mathsf{W} $$.

### Definition

Let $$ \mathsf{T} $$ be a linear operator on a vector space $$ \mathsf{V} $$, and let $$ x $$ be a nonzero vector in $$ \mathsf{V} $$.
The subspace
...
In fact, $$ \mathsf{W} $$ is the "smallest" $$ \mathsf{T} $$-invariant subspace of $$ \mathsf{V} $$ containing $$ x $$.
That is, any $$ \mathsf{T} $$-invariant subspace of $$ \mathsf{V} $$ containing $$ x $$ must also contain $$ \mathsf{W} $$.

### Theorem 5.20

Let $$ \mathsf{T} $$ be a linear operator on a finite-dimensional vector space $$ \mathsf{V} $$, and let $$ \mathsf{W} $$ be a $$ \mathsf{T} $$-invariant subspace of $$ \mathsf{V} $$.
Then the characteristic polynomial of $$ {\mathsf{T}}_{\mathsf{W}} $$ divides the characteristic polynomial of $$ \mathsf{T} $$.

**Proof**  
Choose an ordered basis $$ \gamma = \{ v_1, v_2, \dots, v_k \} $$ for $$ \mathsf{W} $$, and extend it to an ordered basis $$ \beta = \{ v_1, v_2, \dots, v_k, v_{k + 1}, \dots, v_n \} $$ for $$ \mathsf{V} $$.
Let $$ A = {[\mathsf{T}]}_{\beta} $$ and $$ B_1 = {[{\mathsf{T}}_{\mathsf{W}}]}_{\gamma} $$.
...
Let $$ f(t) $$ be the characteristic polynomial of $$ \mathsf{T} $$ and $$ g(t) $$ the characteristic polynomial of $$ {\mathsf{T}}_{\mathsf{W}} $$.
Then
...
Thus $$ g(t) $$ divides $$ f(t) $$. $$ \blacksquare $$

### Theorem 5.21

Let $$ \mathsf{T} $$ be a linear operator on a finite-dimensional vector space $$ \mathsf{V} $$, and let $$ \mathsf{W} $$ denote the $$ \mathsf{T} $$-cyclic subspace of $$ \mathsf{V} $$ generated by a nonzero vector $$ v \in \mathsf{V} $$.
Let $$ k = \dim(\mathsf{W}) $$.
Then  
(a) $$ \{ v, \mathsf{T}(v), {\mathsf{T}}^2(v), \dots, {\mathsf{T}}^{k - 1}(v) \} $$ is a basis for $$ \mathsf{W} $$.  
(b) If $$ a_0 v + a_1 \mathsf{T}(v) + \cdots + a_{k - 1} {\mathsf{T}}^{k - 1}(v) + {\mathsf{T}}^k(v) = \mathit{0} $$, then the characteristic polynomial of $$ {\mathsf{T}}_{\mathsf{W}} $$ is $$ f(t) = {(-1)}^k (a_0 + a_1 t + \cdots + a_{k - 1} t^{k - 1} + t^k) $$.

**Proof**  
(a) Since $$ v \neq \mathit{0} $$, the set $$ \{ v \} $$ is linearly independent.
Let $$ j $$ be the largest positive integer for which

$$
\begin{align*}
    \beta = \{ v, \mathsf{T}(v), \dots, {\mathsf{T}}^{j - 1}(v) \}
\end{align*}
$$

is linearly independent.
Such a $$ j $$ must exist because $$ \mathsf{V} $$ is finite-dimensional.
Let $$ \mathsf{Z} = \text{span}(\beta) $$.
Then $$ \beta $$ is a basis for $$ \mathsf{Z} $$.
Furthermore, $$ {\mathsf{T}}^j(v) \in \mathsf{Z} $$ by Theorem 1.7.
We use this information to show that $$ \mathsf{Z} $$ is a $$ \mathsf{T} $$-invariant subspace of $$ \mathsf{V} $$.
Let $$ w \in \mathsf{Z} $$.
Since $$ w $$ is a linear combination of the vectors of $$ \beta $$, there exist scalars $$ b_0, b_1, \dots, b_{j - 1} $$ such that

$$
\begin{align*}
    w = b_0 v + b_1 \mathsf{T}(v) + \cdots + b_{j - 1} {\mathsf{T}}^{j - 1}(v)
\end{align*}
$$
...
$$
\begin{align*}
    \mathsf{T}(w) = b_0 \mathsf{T}(v) + b_1 {\mathsf{T}}^2(v) + \cdots + b_{j - 1} {\mathsf{T}}^j(v)
\end{align*}
$$

Thus $$ \mathsf{T}(w) $$ is a linear combination of vectors in $$ \mathsf{Z} $$, and hence belongs to $$ \mathsf{Z} $$.
So $$ \mathsf{Z} $$ is $$ \mathsf{T} $$-invariant.
Furthermore, $$ v \in \mathsf{Z} $$ and $$ \mathsf{W} $$ is the smallest $$ \mathsf{T} $$-invariant subspace of $$ \mathsf{V} $$ that contains $$ v $$, so that $$ \mathsf{W} \subseteq \mathsf{Z} $$.
Clearly, $$ \mathsf{Z} \subseteq \mathsf{W} $$, and so we conclude that $$ \mathsf{Z} = \mathsf{W} $$.
It follows that $$ \beta $$ is a basis for $$ \mathsf{W} $$, and therefore $$ \dim(\mathsf{W}) = j $$.
Thus $$ j = k $$.  
(b) Now view $$ \beta $$ from (a) as an ordered basis for $$ \mathsf{W} $$.
Let $$ a_0, a_1, \dots, a_{k - 1} $$ be the scalars such that

$$
\begin{align*}
    a_0 v + a_1 \mathsf{T}(v) + \cdots + a_{k - 1} {\mathsf{T}}^{k - 1}(v) + {\mathsf{T}}^k(v) = \mathit{0}
\end{align*}
$$
...
Thus $$ f(t) $$ is the characteristic polynomial of $$ {\mathsf{T}}_{\mathsf{W}} $$, proving (b). $$ \blacksquare $$